...
path_to_spark="/usr/lib/spark/spark-2.4.4-bin-hadoop2.7"

if [ -n "$2" ]; then path_to_spark=$2; fi

$path_to_spark/bin/spark-submit --conf spark.eventLog.enabled=true --conf spark.eventLog.dir="/tmp" --driver-memory 4g --class paristech.$1 target/scala-2.11/*.jar
